...
export class TestimonyController {
  ...
  static async getByUserId(req: Request, res: Response) {
    try {
      ...
      const testimonies = await testimonyService.getTestimonyByUserId(req.user.id_usuario);
  ...
  static async delete(req: Request, res: Response) {
    try {
      ...
      const id = parseInt(req.params.id);
      await testimonyService.deleteTestimony(id, req.user.id_usuario, req.user.id_rol);
      res.status(204).send();
    } catch (error) {
