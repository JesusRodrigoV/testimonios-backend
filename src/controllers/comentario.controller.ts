  ...
  static async getAll(req: Request, res: Response) {
    try {
      if (req.user?.id_rol === Rol.ADMIN) {
        // solo los admins ven todos los comentarios
        const comentarios = await ComentarioService.findAll();
        return res.json(comentarios);
      }

      const comentarios = await ComentarioService.findApproved(); // si no es admin, solo ve los comentarios aprobados
  ...
  static async create(req: Request, res: Response) {
    try {
      ...
      if (!contenido || !id_testimonio) {
        return res
          .status(400)
          .json({ error: "Contenido e ID de testimonio son requeridos" });
      }
